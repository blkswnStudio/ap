  ...
  [Contracts_ATC.DebtToken.STOCK_2]: {
    ...
    troveMintedAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.DebtToken[Contracts_ATC.DebtToken.STOCK_2].troveMintedAmount.value(
            tokenAmount,
          );
        }
      },
    ...
    providedStability: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.DebtToken[Contracts_ATC.DebtToken.STOCK_2].providedStability.value(
            tokenAmount,
          );
        }
      },
