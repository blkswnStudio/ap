...
const ERC20_GOV = {
  [Contracts_ATC.ERC20.GOV]: {
    priceUSDOracle: {
      fetch: async (updatedPrice?: bigint, priceFeedContract?: PriceFeed) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].priceUSDOracle.lastFetched = Date.now();

        if (priceFeedContract && !updatedPrice) {
          const price = await priceFeedContract.getPrice(Contracts_ATC.ERC20.GOV);
          updatedPrice = BigInt(price.price);
        }

        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].priceUSDOracle.value(updatedPrice);
      },
    ...
    borrowingRate: {
      fetch: async (troveManagerContract: TroveManager) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].borrowingRate.lastFetched = Date.now();

        const borrowingRate = await troveManagerContract.getBorrowingRate(isStableCoinAddress(Contracts_ATC.ERC20.GOV));

        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].borrowingRate.value(borrowingRate);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    walletAmount: {
      fetch: async (collTokenContract: ERC20, depositor: AddressLike) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].walletAmount.lastFetched = Date.now();

        const walletAmount = await collTokenContract.balanceOf(depositor);

        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].walletAmount.value(walletAmount);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    troveLockedAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].troveLockedAmount.lastFetched = Date.now();
        ...
        const tokenAmount = ContractDataFreshnessManager_ATC.TroveManager.getTroveWithdrawableColls.value.find(
          ({ tokenAddress }) => getCheckSum(tokenAddress) === getCheckSum(Contracts_ATC.ERC20.GOV),
        )?.amount;
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].troveLockedAmount.value(tokenAmount);
        }
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
    ...
    stabilityGainedAmount: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].stabilityGainedAmount.lastFetched = Date.now();
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].stabilityGainedAmount.value(tokenAmount);
        }
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
    ...
    collSurplusAmount: {
      fetch: async (fetchSource?: { collSurplusContract: CollSurplusPool; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].collSurplusAmount.lastFetched = Date.now();
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.GOV].collSurplusAmount.value(tokenAmount);
        }
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
