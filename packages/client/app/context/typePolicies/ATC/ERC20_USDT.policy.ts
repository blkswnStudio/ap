...
const ERC20_USDT = {
  [Contracts_ATC.ERC20.USDT]: {
    priceUSDOracle: {
      fetch: async (updatedPrice?: bigint, priceFeedContract?: PriceFeed) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].priceUSDOracle.lastFetched = Date.now();

        if (priceFeedContract && !updatedPrice) {
          const price = await priceFeedContract.getPrice(Contracts_ATC.ERC20.USDT);
          updatedPrice = BigInt(price.price);
        }

        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].priceUSDOracle.value(updatedPrice);
      },
    ...
    borrowingRate: {
      fetch: async (troveManagerContract: TroveManager) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].borrowingRate.lastFetched = Date.now();

        const borrowingRate = await troveManagerContract.getBorrowingRate(
          isStableCoinAddress(Contracts_ATC.ERC20.USDT),
        );

        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].borrowingRate.value(borrowingRate);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    walletAmount: {
      fetch: async (collTokenContract: ERC20, depositor: AddressLike) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].walletAmount.lastFetched = Date.now();

        const walletAmount = await collTokenContract.balanceOf(depositor);

        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].walletAmount.value(walletAmount);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    troveLockedAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].troveLockedAmount.lastFetched = Date.now();
        ...
        const tokenAmount = ContractDataFreshnessManager_ATC.TroveManager.getTroveWithdrawableColls.value.find(
          ({ tokenAddress }) => getCheckSum(tokenAddress) === getCheckSum(Contracts_ATC.ERC20.USDT),
        )?.amount;
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].troveLockedAmount.value(tokenAmount);
        }
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
    ...
    stabilityGainedAmount: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].stabilityGainedAmount.lastFetched = Date.now();
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].stabilityGainedAmount.value(tokenAmount);
        }
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
    ...
    collSurplusAmount: {
      fetch: async (fetchSource?: { collSurplusContract: CollSurplusPool; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].collSurplusAmount.lastFetched = Date.now();
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_ATC.ERC20[Contracts_ATC.ERC20.USDT].collSurplusAmount.value(tokenAmount);
        }
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
