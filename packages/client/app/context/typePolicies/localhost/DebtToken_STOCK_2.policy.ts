...
const DebtToken_STOCK_2 = {
  [Contracts_Localhost.DebtToken.STOCK_2]: {
    priceUSDOracle: {
      fetch: async (updatedPrice?: bigint, priceFeedContract?: PriceFeed) => {
        ...
        if (priceFeedContract && !updatedPrice) {
          const price = await priceFeedContract.getPrice(Contracts_Localhost.DebtToken.STOCK_2);
          updatedPrice = BigInt(price.price);
        }

        SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].priceUSDOracle.value(
          updatedPrice,
        );
      },
    ...
    borrowingRate: {
      fetch: async (troveManagerContract: TroveManager) => {
        ...
        const borrowingRate = await troveManagerContract.getBorrowingRate(
          isStableCoinAddress(Contracts_Localhost.DebtToken.STOCK_2),
        );

        SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].borrowingRate.value(
          borrowingRate,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    walletAmount: {
      fetch: async (debtTokenContract: DebtToken, borrower: AddressLike) => {
        SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].walletAmount.lastFetched =
          Date.now();

        const borrowerBalance = await debtTokenContract.balanceOf(borrower);

        SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].walletAmount.value(
          borrowerBalance,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    troveMintedAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        ...
        const tokenAmount = ContractDataFreshnessManager_LOCALHOST.TroveManager.getTroveDebt.value.find(
          ({ tokenAddress }) => getCheckSum(tokenAddress) === getCheckSum(Contracts_Localhost.DebtToken.STOCK_2),
        )?.amount;
        if (tokenAmount) {
          SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].troveMintedAmount.value(
            tokenAmount,
          );
        }
      },
    ...
    troveDebtAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        ...
        if (repayableDebt) {
          SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].troveDebtAmount.value(
            repayableDebt,
          );
        }
      },
    ...
    providedStability: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        ...
        if (tokenAmount) {
          SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].providedStability.value(
            tokenAmount,
          );
        }
      },
    ...
    compoundedDeposit: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        ...
        if (compoundedDeposit) {
          SchemaDataFreshnessManager_LOCALHOST.DebtToken[Contracts_Localhost.DebtToken.STOCK_2].compoundedDeposit.value(
            compoundedDeposit,
          );
        }
      },
