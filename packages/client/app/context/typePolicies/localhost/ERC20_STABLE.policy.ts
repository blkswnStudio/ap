...
const ERC20_STABLE = {
  [Contracts_Localhost.ERC20.STABLE]: {
    priceUSDOracle: {
      fetch: async (updatedPrice?: bigint, priceFeedContract?: PriceFeed) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].priceUSDOracle.lastFetched =
          Date.now();

        if (priceFeedContract && !updatedPrice) {
          const price = await priceFeedContract.getPrice(Contracts_Localhost.ERC20.STABLE);
          updatedPrice = BigInt(price.price);
        }

        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].priceUSDOracle.value(updatedPrice);
      },
    ...
    borrowingRate: {
      fetch: async (troveManagerContract: TroveManager, cache: InMemoryCache, borrower: string) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].borrowingRate.lastFetched =
          Date.now();
        ...
        const borrowingRate = await troveManagerContract.getBorrowingRate(
          isStableCoinAddress(Contracts_Localhost.ERC20.STABLE),
          percentageGovToken,
        );

        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].borrowingRate.value(borrowingRate);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    walletAmount: {
      fetch: async (collTokenContract: ERC20, depositor: AddressLike) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].walletAmount.lastFetched =
          Date.now();

        const walletAmount = await collTokenContract.balanceOf(depositor);

        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].walletAmount.value(walletAmount);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    troveLockedAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].troveLockedAmount.lastFetched =
          Date.now();
        ...
        const tokenAmount =
          ContractDataFreshnessManager_LOCALHOST.TroveManager.getTroveWithdrawableColls.value.find(
            ({ tokenAddress }) => getCheckSum(tokenAddress) === getCheckSum(Contracts_Localhost.ERC20.STABLE),
          )?.amount ?? defaultFieldValue;
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].troveLockedAmount.value(
          tokenAmount,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    stabilityGainedAmount: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].stabilityGainedAmount.lastFetched =
          Date.now();
        ...
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].stabilityGainedAmount.value(
          tokenAmount,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    collSurplusAmount: {
      fetch: async (fetchSource?: { collSurplusContract: CollSurplusPool; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].collSurplusAmount.lastFetched =
          Date.now();
        ...
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STABLE].collSurplusAmount.value(
          tokenAmount,
        );
      },
      value: makeVar(defaultFieldValue),
