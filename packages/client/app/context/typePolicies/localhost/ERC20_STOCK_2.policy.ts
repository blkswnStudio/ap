...
const ERC20_STOCK_2 = {
  [Contracts_Localhost.ERC20.STOCK_2]: {
    priceUSDOracle: {
      fetch: async (updatedPrice?: bigint, priceFeedContract?: PriceFeed) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].priceUSDOracle.lastFetched =
          Date.now();

        if (priceFeedContract && !updatedPrice) {
          const price = await priceFeedContract.getPrice(Contracts_Localhost.ERC20.STOCK_2);
          updatedPrice = BigInt(price.price);
        }

        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].priceUSDOracle.value(
          updatedPrice,
        );
      },
    ...
    borrowingRate: {
      fetch: async (troveManagerContract: TroveManager, cache: InMemoryCache, borrower: string) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].borrowingRate.lastFetched =
          Date.now();
        ...
        const borrowingRate = await troveManagerContract.getBorrowingRate(
          isStableCoinAddress(Contracts_Localhost.ERC20.STOCK_2),
          percentageGovToken,
        );

        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].borrowingRate.value(
          borrowingRate,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    walletAmount: {
      fetch: async (collTokenContract: ERC20, depositor: AddressLike) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].walletAmount.lastFetched =
          Date.now();

        const walletAmount = await collTokenContract.balanceOf(depositor);

        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].walletAmount.value(walletAmount);
      },
      value: makeVar(defaultFieldValue),
      lastFetched: 0,
      timeout: 1000 * 2,
    ...
    troveLockedAmount: {
      fetch: async (fetchSource?: { troveManagerContract: TroveManager; borrower: AddressLike }) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].troveLockedAmount.lastFetched =
          Date.now();
        ...
        const tokenAmount =
          ContractDataFreshnessManager_LOCALHOST.TroveManager.getTroveWithdrawableColls.value.find(
            ({ tokenAddress }) => getCheckSum(tokenAddress) === getCheckSum(Contracts_Localhost.ERC20.STOCK_2),
          )?.amount ?? defaultFieldValue;
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].troveLockedAmount.value(
          tokenAmount,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    stabilityGainedAmount: {
      fetch: async (fetchSource?: { stabilityPoolManagerContract: StabilityPoolManager; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[
          Contracts_Localhost.ERC20.STOCK_2
        ].stabilityGainedAmount.lastFetched = Date.now();
        ...
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].stabilityGainedAmount.value(
          tokenAmount,
        );
      },
      value: makeVar(defaultFieldValue),
    ...
    collSurplusAmount: {
      fetch: async (fetchSource?: { collSurplusContract: CollSurplusPool; depositor: AddressLike }) => {
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].collSurplusAmount.lastFetched =
          Date.now();
        ...
        SchemaDataFreshnessManager_LOCALHOST.ERC20[Contracts_Localhost.ERC20.STOCK_2].collSurplusAmount.value(
          tokenAmount,
        );
      },
      value: makeVar(defaultFieldValue),
