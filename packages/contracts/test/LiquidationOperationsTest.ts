...
describe('LiquidationOperations', () => {
  ...
  describe('in Normal Mode', () => {
    describe('liquidate()', () => {
      ...
      it('liquidates based on entire collateral/debt (including pending rewards), not raw collateral/debt', async () => {
        ...
        expect(d2ICRAfter).to.be.gt(d2ICRBefore); // gets a little bit higher/better because the 200 stable gas comp gets removed as debt from the defaulter, which results in a >110% ICR of the trove even after liquidation
      ...
      it("liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and collateral gain", async () => {
        ...
        const dennisICR = (await hintHelpers.getCurrentICR(dennis)).ICR;
      ...
      it('Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves', async () => {
        await openTrove({
          from: whale,
          contracts,
          colls: [{ tokenAddress: BTC, amount: parseUnits('30', 8) }],
        ...
        await openTrove({
          from: defaulter_2,
          contracts,
          colls: [{ tokenAddress: BTC, amount: parseUnits('0.5', 8) }],
          debts: [{ tokenAddress: STABLE, amount: parseUnits('8000') }],
        });
  ...
  describe('batchLiquidateTroves()', () => {
    ...
    it('does not close troves with ICR >= MCR in the given array', async () => {
      ...
      await whaleShrimpTroveInit(contracts, signers, false, contracts.STOCK);
  ...
  describe('in Recovery Mode', () => {
    ...
    describe('liquidate()', () => {
      ...
      it('recovery mode, with  110% < ICR < TCR, full stability pool off set', async () => {
        ...
        //check totals
        const totalCollateralSnapshot_After = await troveManager.totalCollateralSnapshots(BTC);
        assert.equal(totalCollateralSnapshot_After, parseUnits('4', 8)); // from the remaining two open troves
        ...
        expect(bobCollWithoutGasComp).to.be.closeTo(aliceSP[0][1] + collSurplus[0].amount, 5000); // tolerance in the stability pool stake calculation
      });
    ...
    describe('batchLiquidateTroves()', () => {
      it('Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode', async () => {
        ...
        //batchliquidate bob
        await batchLiquidate([dennis, alice, defaulter_1, defaulter_2, bob, carol], contracts);
