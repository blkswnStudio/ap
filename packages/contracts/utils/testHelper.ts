...
export const increaseDebt = async (
  ...
  const [upperHint, lowerHint] = await getHints(contracts, afterPathCR);
  const od = await generatePriceUpdateDataWithFee(contracts, priceDataUpdateOffset);
  return {
...
export const whaleShrimpTroveInit = async (
  ...
  if (depositStability)
    await stabilityPoolManager
      .connect(alice)
      .provideStability([{ tokenAddress: debtToken, amount: parseUnits('1000') }]);
  await openTrove({
    from: bob,
    contracts,
    colls: [{ tokenAddress: BTC, amount: parseUnits('1', 8) }],
  ...
  if (depositStability)
    await stabilityPoolManager.connect(bob).provideStability([{ tokenAddress: debtToken, amount: parseUnits('2000') }]);
  await openTrove({
    from: carol,
    contracts,
    colls: [{ tokenAddress: BTC, amount: parseUnits('1', 8) }],
  ...
  if (depositStability)
    await stabilityPoolManager
      .connect(carol)
      .provideStability([{ tokenAddress: debtToken, amount: parseUnits('3000') }]);
