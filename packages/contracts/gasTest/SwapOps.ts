...
import { MockDebtToken, MockERC20, TroveManager, SwapPair, SwapOperations, MockPyth } from '../typechain';
import { expect } from 'chai';
import { openTrove, getLatestBlockTimestamp, deployTesting } from '../utils/testHelper';
...
describe('SwapOperations', () => {
  ...
  beforeEach(async () => {
    ...
    await swapOperations.connect(owner).createPair(STABLE, STOCK);
  ...
  describe('Add Liquidity', () => {
    makeDescribe('without borrow', async (accs: SignerWithAddress[], funcs: MakeDescribeFunctions) => {
      ...
      for (const acc of accs) {
        ...
        expect(await pair.balanceOf(acc)).to.be.greaterThan(0);
  ...
  describe('Remove Liquidity', () => {
    makeDescribe('without repay', async (accs: SignerWithAddress[], funcs: MakeDescribeFunctions) => {
      ...
      for (const acc of accs) {
        ...
        //remove liquidty
        const tx = await remove(acc, STOCK, await pair.balanceOf(acc));
        expect(await pair.balanceOf(acc)).to.be.equal(0);
    ...
    makeDescribe('with repay', async (accs: SignerWithAddress[], funcs: MakeDescribeFunctions) => {
      ...
      for (const acc of accs) {
        ...
        const tx = await remove(acc, STOCK, await pair.balanceOf(acc));
        expect(await pair.balanceOf(acc)).to.be.equal(0);
        expect(await troveManager.getTroveRepayableDebt(acc, STABLE, false)).to.be.equal(0);
