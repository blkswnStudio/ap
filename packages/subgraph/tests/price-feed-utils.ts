...
export const mockPriceFeed_getPrice = (
  ...
  createMockedFunction(MockPriceFeedAddress, 'getPrice', 'getPrice(address):(uint256,bool)')
    .withArgs([ethereum.Value.fromAddress(tokenAddress)])
    // This is how you mock an object value
    .returns([ethereum.Value.fromUnsignedBigInt(value), ethereum.Value.fromBoolean(true)]);
};
