...
describe('handleTransfer()', () => {
  ...
  describe('handleUpdateTradingUserPosition_onTransfer()', () => {
    ...
    test('mint for generic DebtToken', () => {
      mockPriceFeed_getUSDValue(MockDebtToken_STABLE_Address, oneEther);
      handleTransfer(createTransferEvent(ZeroAddress, MockUserAddress, oneEther, MockDebtToken_STABLE_Address));

      const entityId = `${MockUserAddress.toHexString()}-${MockDebtToken_STABLE_Address.toHexString()}`;
    ...
    test('transfer away generic DebtToken', () => {
      const mintAmount = oneEther.times(BigInt.fromI32(2));
      const entityId = `${MockUserAddress.toHexString()}-${MockDebtToken_STABLE_Address.toHexString()}`;

      // first mint 2 ether
      mockPriceFeed_getUSDValue(MockDebtToken_STABLE_Address, mintAmount);
      handleTransfer(createTransferEvent(ZeroAddress, MockUserAddress, mintAmount, MockDebtToken_STABLE_Address));
      ...
      handleTransfer(
        createTransferEvent(MockUserAddress, MockSecondUserAddress, oneEther, MockDebtToken_STABLE_Address),
      );
      assert.fieldEquals('TradingUserPosition', entityId, 'amount', oneEther.toString());
      assert.fieldEquals('TradingUserPosition', entityId, 'value', oneEther.toString());
